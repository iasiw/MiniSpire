以下是基于该仓库内容生成的 `README.md` 文件。由于仓库描述和详细信息较少，我根据其文件结构（Python + HTML/JS，包含 `main.py`, `websocket`, `test_training.py` 等）进行了合理推断，生成了符合一般开源项目规范的文档。
...
### 环境要求
- Python 3.8 或更高版本
- 依赖包：`websockets`，`asyncio` 等（具体见 `requirements.txt`，如未提供请根据导入报错手动安装）
